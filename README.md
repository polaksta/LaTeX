...
|With the 'parttitle' option, you can specify whether the part name should be displayed in the left (`\usetheme[parttitle=leftfooter]{AGH}`) or right footer `(\usetheme[parttitle=rightfooter]{AGH}`) - based on my own experience, I can say that the division into parts is very indicated in the case of big presentations, such as e.g., lectures.|Za pomocą opcji 'parttitle' można określić czy nazwa części ma być wyświetlana w lewej (`\usetheme[parttitle=leftfooter]{AGH}`) czy w prawej stopce (`\usetheme[parttitle=rightfooter]{AGH}`) - na podstawie własnego doświadczenia mogę stwierdzić, że podział na części jest bardzo wskazany w przypadku dużych prezentacji, takich jak na przykład wykłady.|
