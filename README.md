...
| The default size of the left and right margins of the text area is 2em. If you want to set your own value, use the *margins* option, assigning its the desired margin value, eg `\ usetheme [margins = 1em] {AGH}` | Domyślny rozmiar lewego i prawego marginesu obszaru tekstu wynosi 2em. Jeżeli chcesz ustawić własną wartość, to użyj opcji *margins*, przypisując jej żądaną wartość marginesu, np. `\usetheme[margins=1em]{AGH}` |
